select region,''''||lsm lsm,confidence, case when current_date -lastinetractiondate <= 90 then 'less than 3 months'
...
from (select region,lsm,confidence,mobile_number,max(lastinetractiondate) lastinetractiondate
from GAIN_THEORY.PRMDashboard_data group by 1,2,3,4) a group by 1,2,3,4;
...
from (select region,lsm,confidence,mobile_number,max(lastinetractiondate) lastinetractiondate
from GAIN_THEORY.PRMDashboard_data where lower(brand) like '%cornet%' group by 1,2,3,4) a group by 1,2,3,4;
...
from GAIN_THEORY.PRMDashboard_data 
where lsm in ('4-7','7+') and lower(brand) like '%cornet%' group by 1,2,3,4) a group by 1,2,3,4
having count(distinct mobile_number) > 100;

--pincode latlong region wise
select region,count(distinct mobile_number)
from GAIN_THEORY.PRMDashboard_data
where length(pincode) > 7 and pincode not like '%0.0.0%'
group by 1;
...
select count(distinct mobile_number)  from GAIN_THEORY.PRMDashboard_data
where length(pincode) = 6 and pincode <> '000000';